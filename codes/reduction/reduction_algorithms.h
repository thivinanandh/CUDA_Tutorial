...
// Reduction 1
// This is the basic reduction algorithm
// 1, Allow threads which are multiples of the strides at each stage to perform the reduction
// 2, Sync the threads at each stage
// Cons: This will introduce thread divergence at each stage. Since within a Given warp, threads are supposed to be in sync
//  However, here within a warp (say 32), only 16 of them are active at first stage, causing warp divergence
...
__global__ void reduction_2(float *g_idata, float *g_odata)
{
    // Copy the corresponding arrays to the shared memory
    extern __shared__ float sdata[];
...
__global__ void reduction_3(float *g_idata, float *g_odata)
{
    // Copy the corresponding arrays to the shared memory
    extern __shared__ float sdata[];
...
__global__ void reduction_4(float *g_idata, float *g_odata)
{
    // Copy the corresponding arrays to the shared memory
    extern __shared__ float sdata[];
...
__global__ void reduction_5(float *g_idata, float *g_odata)
{
    // Copy the corresponding arrays to the shared memory
    extern __shared__ float sdata[];
